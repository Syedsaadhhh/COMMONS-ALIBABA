...
import type { Coordinates, ProjectBundle, TaskRecord } from "@/lib/projects/types";
      ...
      <div className="shell project-workspace">
        ...
        <div className="workspace-grid">
          ...
          <section className="workspace-panel workspace-panel--wide">
            ...
            <form className="evidence-form" onSubmit={submitEvidence}><label>Evidence title<input className="form-control" value={evidenceTitle} onChange={(event) => setEvidenceTitle(event.target.value)} placeholder="Morning traffic observation" required /></label><label>Source link<input className="form-control" type="url" value={evidenceUrl} onChange={(event) => setEvidenceUrl(event.target.value)} placeholder="https://…" required /></label><label className="evidence-form__full">Context (optional)<textarea className="form-control" value={evidenceDescription} onChange={(event) => setEvidenceDescription(event.target.value)} placeholder="What does this source show, and when was it observed?" rows={3} /></label><div className="evidence-form__actions"><Button type="button" variant="outline" size="sm" disabled={busy === "evidence-location"} onClick={() => void captureEvidenceLocation()}>{evidenceCoordinates ? "Evidence location captured" : busy === "evidence-location" ? "Capturing…" : "Add my evidence location"}</Button><Button type="submit" disabled={busy === "evidence"}>{busy === "evidence" ? "Saving…" : "Submit evidence"}</Button></div><p className="input-note evidence-form__full">COMMONS stores the source URL and a fingerprint of that URL for reference. It does not claim to verify the source content.</p></form>
            <div className="evidence-records">{evidence.length ? evidence.map((item) => <article key={item.id}><div><h3>{item.title}</h3><p>{item.description || "No additional context"}</p><small>Submitted {formatDate(item.created_at)} · {item.latitude !== null ? "Location captured" : "No location pin"}</small></div><div><span className="draft-state">{item.status.replaceAll("_", " ")}</span><a className="plain-link" href={item.file_url} target="_blank" rel="noreferrer">Open source</a></div></article>) : <p className="empty-copy">No evidence has been submitted. Add a link that a reviewer can open.</p>}</div>
          </section>
        </div>
      </div>
    </main>
